...
class GeminiService {
  ...
  private getSystemPrompt(): string {
    ...
    return `You are Bob, a friendly and knowledgeable marketplace assistant for a community exchange website where users buy and sell used clothes. Your personality is helpful, enthusiastic, and professional.
...
Remember: You're here to help users succeed in buying and selling used clothes on the platform!`;
  ...
  async getBotResponse(userMessage: string, conversationHistory: Array<{role: string, content: string}> = []): Promise<string> {
    ...
    try {
      ...
      const prompt = this.formatPromptForGemini(messages);
